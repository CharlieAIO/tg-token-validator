...
const failed_message = (amount:string) => `Sorry you still need ${amount} tokens to access the Degenerate Whale chat. Your tokens will be sent back;`;
        ...
        for (const instruction of txDetails.transaction.message.instructions) {
            if (instruction.program == "system") {
                ...
                if (!confirmed || !has_holdings) {
                    await bot.sendMessage(chatId, returnFunds?failed_message(tokens_required_remaining.toFixed(2)):"Access Denied.")
